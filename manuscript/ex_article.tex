...
$\psi$ satisfies~\eqref{weak_form}, which implies that $\vertiii{\mathcal{R}^k(\psi)}_{k^\ast}=0$. Thanks to the identity~\eqref{res_error}, we obtain that $\psi=n^{k}$ in $X_k$. This section concludes the proof of Theorem~\ref{thm:wellposed}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
The figure \ref{fig1:bi} below shows the splitting of an example density region in 2D, where $x_i^k = (0,0)$ and $\upsilon_i^k = \diag(\upsilon_{i,11}^k, \upsilon_{i,22}^k)$.
\begin{figure}[h]
\centering
...
The preceding developments leads to the following result.
...
With the parameters given in the previous section, we employ both the heuristic and multi-scale speciation algorithms. The figures below shows the species abundance~\ref{ex1:abundance}, the species mean trait coordinates~\ref{ex1:x1}, the largest eigenvalue of the trait covariance matrix~\ref{ex1:variance}, the a posteriori error bound~\ref{ex1:residuals}, and the error in energy norm~\ref{ex1:edist}, as functions of time. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Simulation}
With the parameters given in the previous section, we employ both the heuristic and multi-scale algorithms. The figures below shows the species abundance~\ref{ex2:prey_abundance}--\ref{ex2:pred_abundance}, the species mean trait coordinates~\ref{ex2:prey_x1}--\ref{ex2:prey_x2}, the largest eigenvalue of the trait covariance matrix~\ref{ex2:prey_variance}, and the a posteriori error bounds~\ref{ex2:prey_residuals}--\ref{ex2:pred_residuals}, as functions of time. 
